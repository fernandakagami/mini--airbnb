	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogTrigger asChild>
				<Button className="sr-only !w-24 !py-2 md:not-sr-only" variant="outline">
					<Filter size={20} />
					Filtros
				</Button>
			</DialogTrigger>
			<DialogContent
				className="max-h-screen overflow-y-auto sm:max-w-md"
				aria-describedby="Modal para reservar uma acomodação"
			>
				<DialogHeader>
					<DialogTitle>Filtros de Busca</DialogTitle>
					<DialogDescription>Refine sua busca de acomodações com os filtros abaixo.</DialogDescription>
				</DialogHeader>
				<Form {...formMethods}>
					<form
						onSubmit={handleSubmit((data) => handleSearch(data))}
						className="flex w-full flex-col items-end justify-center gap-3"
					>
				...
				<DialogFooter>
					<DialogClose asChild>
						<Button variant="secondary" className="w-full" onClick={clearForm}>
							Limpar Filtro
						</Button>
					</DialogClose>
					<DialogClose asChild>
